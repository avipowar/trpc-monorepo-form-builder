...
export default function SubmissionsPage() {
  ...
  if (error) {
    return (
      <div className="flex min-h-[80vh] w-full items-center justify-center p-4">
        <div className="w-full max-w-md rounded-2xl border border-red-500/20 bg-red-500/5 dark:bg-red-500/10 p-6 text-center shadow-xl">
  ...
  if (!allSubmissions || allSubmissions.length === 0) {
    return (
      <div className="flex min-h-[80vh] w-full items-center justify-center p-4">
        <div className="w-full max-w-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl p-8 flex flex-col items-center justify-center space-y-6 shadow-xl text-center">
          <div className="h-16 w-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-500 shadow-inner">
            <Inbox className="h-8 w-8 animate-pulse" />
          </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 py-8 px-4 sm:px-6 lg:px-8 pb-24 transition-colors duration-300">
      <div className="max-w-6xl mx-auto space-y-8 w-full">
        <div className="flex items-start justify-between pb-6 w-full border-b border-zinc-200 dark:border-zinc-800">
          <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          {allSubmissions.map((submission) => {
            ...
            return (
              <div
                key={submission.id}
                className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 rounded-2xl shadow-sm dark:shadow-xl hover:border-zinc-300 dark:hover:border-zinc-700 transition-all duration-200 flex flex-col justify-between space-y-4 group"
              >
